...
| **A05 Security Misconfiguration**                | 0                           | Container app roda como `node` (não-root); frontends usam `nginxinc/nginx-unprivileged` (uid 101); rate limiting via `@nestjs/throttler`; CORS restrito; Helmet aplicável (recomendado em prod) | ✅ Mitigado                        |
...
| **A07 Identification & Authentication Failures** | 0                           | JWT com expiração curta; bcrypt; `@Throttle` no `/auth/login` recomendado para prod (rate-limit global já ativo)                                                                                | ✅ Mitigado                        |
