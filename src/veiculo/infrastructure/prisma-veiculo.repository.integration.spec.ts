...
describe("PrismaVeiculoRepository (integration)", () => {
  ...
  describe("findAll", () => {
    beforeEach(async () => {
      await repository.create(
        buildVeiculo({ placa: "AAA1A11", marca: "Toyota", modelo: "Corolla" }),
      );
      await repository.create(
        buildVeiculo({ placa: "BBB2B22", marca: "Honda", modelo: "Civic" }),
      );
      await repository.create(
        buildVeiculo({ placa: "CCC3C33", marca: "Toyota", modelo: "Hilux" }),
      );
    });
    ...
    it("should filter by marca (case-insensitive)", async () => {
      ...
      result.data.forEach((v) =>
        expect(v.marca.toLowerCase()).toContain("toyota"),
      );
    });
  ...
  describe("findByClienteId", () => {
    ...
    it("should return empty array when cliente has no veiculos", async () => {
      const result = await repository.findByClienteId(
        "00000000-0000-0000-0000-000000000000",
      );
