...
export class OrdemDeServicoController {
  ...
  @ApiOperation({
    summary:
      "Listar ordens de servico com paginacao, filtros e ordenacao customizada",
    description:
      "Retorna ordens de serviço ordenadas por prioridade (EM_EXECUCAO > AGUARDANDO_APROVACAO > EM_DIAGNOSTICO > RECEBIDA). Dentro do mesmo status, ordena por data de criação (mais antigas primeiro). OS com status FINALIZADA ou ENTREGUE são excluídas por padrão, use incluirEncerradas=true para incluí-las.",
  })
  ...
  async findAll(@Query() query: QueryOrdemDeServicoDto) {
    const result = await this.listarOrdensDeServico.execute({
      ...
      status: query.status,
      numero: query.numero,
      incluirEncerradas: query.incluirEncerradas,
    });
  ...
  @ApiCreatedResponse({
    description: "Produto adicionado ao servico com sucesso",
  })
  @ApiNotFoundResponse({
