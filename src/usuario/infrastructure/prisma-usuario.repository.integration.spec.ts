...
describe("PrismaUsuarioRepository — usuario module (integration)", () => {
  ...
  const buildUsuario = (
    overrides: Partial<{ nome: string; email: string; role: Role }> = {},
  ) =>
    Usuario.create({
  ...
  describe("findAll", () => {
    beforeEach(async () => {
      await repository.create(
        buildUsuario({ email: "admin@of.com", role: Role.ADMIN }),
      );
      await repository.create(
        buildUsuario({ email: "mec1@of.com", role: Role.MECANICO }),
      );
      await repository.create(
        buildUsuario({ email: "mec2@of.com", role: Role.MECANICO }),
      );
      await repository.create(
        buildUsuario({
          email: "cli@of.com",
