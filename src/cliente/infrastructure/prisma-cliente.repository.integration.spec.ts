...
describe("PrismaClienteRepository (integration)", () => {
  ...
  const buildCliente = (
    overrides: Partial<{ nome: string; cpfCnpj: string; email: string }> = {},
  ) =>
    Cliente.create({
  ...
  describe("create", () => {
    ...
    it("should persist email when provided", async () => {
      const result = await repository.create(
        buildCliente({ email: "joao@test.com" }),
      );
  ...
  describe("existsByCpfCnpj", () => {
    ...
    it("should exclude a specific id from the check", async () => {
      ...
      const exists = await repository.existsByCpfCnpj(
        "52998224725",
        created.id!,
      );
      expect(exists).toBe(false);
  ...
  describe("findAll", () => {
    beforeEach(async () => {
      await repository.create(
        buildCliente({ nome: "Ana Lima", cpfCnpj: "529.982.247-25" }),
      );
      await repository.create(
        buildCliente({ nome: "Bruno Costa", cpfCnpj: "261.281.281-49" }),
      );
      await repository.create(
        buildCliente({ nome: "Carla Melo", cpfCnpj: "514.234.103-19" }),
      );
    });
